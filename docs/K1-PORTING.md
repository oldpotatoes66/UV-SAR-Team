...
K1 v0.7-dev1 开始恢复 SAR/Bandscope：SAR-TEAM 预设重新编译 K1 原生 Bandscope 引擎，作为后续移植 SAR 目标状态机、峰值保留和近场告警的稳定基线。该开发版必须先确认容量和真机频谱扫描，不得标记为现场验证版。

K1 v0.7-dev2 根据真机照片简化 Bandscope 布局：状态栏仅显示 `BS AUTO/MAN`与电量，主行居中显示峰值频率，底行只保留扫描步进、点数和监听带宽，移除互相重叠的起点/终点和调制文字。

K1 v0.8-dev2 增加面向非专业人员的极简 SAR 测向页：`F + 5` 使用当前接收频率直接进入，自动校准后显示大型强度条和 `TARGET / WAIT / NO SIGNAL / TOO CLOSE` 状态；PTT 只清除峰值、绝不发射，`MENU` 进入专业 Bandscope，`EXIT` 一键退出。实机照片回归后进一步修正底部提示裁切，并过滤接收机启动瞬间的异常峰值。

K1 v0.9-dev1 增加与 TEAM/ARTS 完全分离的测向训练信标。`F + 7` 进入独立页面，`MENU` 启动/停止，默认每 30 秒发送一次呼号 CW 标识和 3 秒 DCS 稳定载波；训练期间临时使用 L1，退出后恢复原功率。低电、非法发射频率或非 FM 模式禁止启动，信道忙时延迟 5 秒重试，30 分钟后自动停止，`EXIT` 可随时中止。

K1 v0.9-dev2 修复训练信标始终显示 `CHANNEL BUSY`：不再用不同机器误差很大的固定 RSSI 数值判断，而是跟随 K1 自身经过校准的载波静噪开/关事件。
