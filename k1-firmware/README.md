...
This branch includes the tested UV-K1 SAR-TEAM port. Hold digit `6` to enter TEAM; `F + 6` changes, saves, and applies actual transmit power. A/B permanently show independent `L1–L5 / M / H` values, with a temporary large confirmation after a change. Yaesu ARTS-compatible DCS 023N monitoring works alongside normal voice RX/PTT TX. AUTO TX is off by default and requires holding `3`. Inside TEAM, key `4` cycles the actual `L1–L5 / M / H` channel power used by voice, ARTS polls, and CW. Low-battery TX lockout, a 60-second manual PTT timeout, repeated lost-link alarms, keypad lock, carrier-busy avoidance, immediate `EXIT` abort, and per-radio callsign configuration are included. See the parent [`README.md`](../README.md) and [`docs/FEATURES.md`](../docs/FEATURES.md) for the canonical controls.

本分支包含 UV-K1 SAR-TEAM 移植版：长按数字 `6` 进入 TEAM，`F + 6` 切换、保存并应用实际发射功率；A/B 两行持续显示各自 `L1–L5 / M / H`，调整时有大字确认。ARTS 与正常语音收发可同时工作。AUTO TX 默认关闭，必须长按 `3` 明确开启；TEAM 内按 `4` 循环 `L1–L5/M/H`，语音、ARTS 轮询和 CW 均使用该实际频道功率。同时包含低电禁发、人工 PTT 60 秒超时、重复失联告警、载波避让、`EXIT` 立即中止、锁键和每机呼号配置。统一按键说明见父仓库 [`README.md`](../README.md) 与 [`docs/FEATURES.md`](../docs/FEATURES.md)。

```sh
./compile-with-docker.sh SarTeam
```

Output / 输出：`build/SarTeam/sar-team-k1.bin`。

For DFU, connect a USB-A-to-USB-C data cable, keep the radio off, then hold PTT while powering on. The screen remains black. Do not hold a side key, which may enter `AIR COPY`. The tested bootloader is `7.02.02`. Never cross-flash this PY32F071 image and a DP32G030 K5/K6 image.

进入 DFU：使用 USB-A→USB-C 数据线，关机后按住 PTT 开机，屏幕保持黑色。不要同时按侧键，否则可能进入 `AIR COPY`。已测试 Bootloader 为 `7.02.02`。本 PY32F071 固件与 K5/K6 DP32G030 固件禁止交叉刷写。

Configure the callsign while the radio is running normally from the parent repository / 正常开机后在父仓库配置呼号：

```sh
python3 ../tools/team_config.py --port /dev/cu.usbmodemXXXXXXXX --model k1 show
```

See [`../docs/K1-PORTING.md`](../docs/K1-PORTING.md) for detailed instructions.
