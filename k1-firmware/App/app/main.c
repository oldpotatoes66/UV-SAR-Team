...
static void processFKeyFunction(const KEY_Code_t Key, const bool beep)
{
    ...
    switch (Key) {
        ...
        case KEY_6:
#ifdef ENABLE_TEAM_MODE
            // Preserve the stock F+6 power shortcut. A deliberate long press
            // of digit 6 enters TEAM/ARTS without a modifier.
            if (beep) {
                TEAM_Run();
                gRequestDisplayScreen = DISPLAY_MAIN;
                break;
...
{
    // On K1 the F latch can be cleared between the press and release events
    // of a digit key.  Latch F+6 on press, then run the normal power action
    // on release so the channel is saved before the VFO is reconfigured.
    static bool f6PowerPending = false;

    if (bKeyHeld) { // key held down
        if (Key == KEY_6 && f6PowerPending)
            return;
