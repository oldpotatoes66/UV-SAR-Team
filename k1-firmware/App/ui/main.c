...
void UI_DisplayMain(void)
{
    ...
    if (gPowerDisplayCountdown_500ms > 0) {
        static const char *const powerLabels[] = {
            "USER", "LOW 1", "LOW 2", "LOW 3",
            "LOW 4", "LOW 5", "MID", "HIGH"
        };
        uint8_t power = gTxVfo->OUTPUT_POWER;

        if (power > OUTPUT_POWER_HIGH)
            power = OUTPUT_POWER_LOW1;

        UI_PrintString("TX POWER", 0, LCD_WIDTH, 1, 8);
        UI_PrintString(powerLabels[power], 0, LCD_WIDTH, 3, 8);
        ST7565_BlitFullScreen();
        return;
    }
    ...
    if (isMainOnly())
    {
        ...
        {   // show the TX power
            ...
            if (gSetting_set_gui)
            {
                ...
                // The K6 renderer deliberately used X > LCD_WIDTH to advance
                // into the next framebuffer row.  K1 bounds-checks X, so use
                // the equivalent explicit row and visible X coordinate.
