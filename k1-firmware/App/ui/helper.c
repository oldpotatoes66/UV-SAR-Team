...
void UI_PrintString(const char *pString, uint8_t Start, uint8_t End, uint8_t Line, uint8_t Width)
{
    ...
    if (Line + 1 >= FRAME_LINES || Start >= LCD_WIDTH || Width == 0)
        return;
    ...
    if (End > Start) {
        const size_t area_width = (size_t)End - Start + 1;
        const size_t text_width = Length * Width;
        if (text_width < area_width)
            Start += (uint8_t)((area_width - text_width) / 2);
    }

    for (i = 0; i < Length; i++)
    {
        const unsigned int ofs   = (unsigned int)Start + (i * Width);
        if (ofs >= LCD_WIDTH || ofs + 7 > LCD_WIDTH)
            break;
        if (pString[i] > ' ' && pString[i] < 127)
        {
            const unsigned int index = pString[i] - ' ' - 1;
...
void UI_PrintStringSmall(const char *pString, uint8_t Start, uint8_t End, uint8_t Line, uint8_t char_width, const uint8_t *font)
{
    ...
    if (Line >= FRAME_LINES || Start >= LCD_WIDTH || char_width == 0)
        return;
    ...
    if (End > Start) {
        const size_t area_width = (size_t)End - Start + 1;
        const size_t text_width = Length * char_spacing;
        if (text_width < area_width)
            Start += (uint8_t)((area_width - text_width) / 2);
    }

    const size_t available = LCD_WIDTH - Start;
    const size_t max_chars = available / char_spacing;
    if (Length > max_chars)
        Length = max_chars;
    ...
    for (size_t i = 0; i < Length; i++) {
        if (pString[i] > ' ' && pString[i] < 127) {
            const unsigned int index = pString[i] - ' ' - 1;
            const size_t offset = Start + i * char_spacing + 1;
            if (offset + char_width > LCD_WIDTH)
                break;
            memcpy(gFrameBuffer[Line] + offset, font + index * char_width, char_width);
        }
    }
}
