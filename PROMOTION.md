...
它把 SAR 无线电测向、Yaesu ARTS 兼容团队链路和日常语音通信整合在同一台电台中：`F + 5` 进入 SAR；长按数字 `6` 进入 TEAM；`F + 6` 仍用于切换发射功率。TEAM 通过 DCS 023N 与 VX-6R、VX-7R、VX-8R 进行低功率轮询，同时保留正常语音收发。失联约 60 秒后发出三次低音告警，并在持续失联时定时重复，恢复连接时发出双高音提示。

公开固件不包含任何个人呼号。救援队可以使用 `team_config.py` 为每台设备单独设置 3–6 位呼号、15/25 秒轮询间隔、P1/P2/P3、提示音和 CW 默认状态，也可以使用 CSV 名册批量部署。配置脚本会自动备份原值、写后复读，并进行 CRC-8 校验。
...
It combines radio direction finding, a Yaesu ARTS-compatible team link, and normal voice communication in one handheld. Press `F + 5` for SAR, hold digit `6` for TEAM, and use `F + 6` to change transmit power. TEAM provides DCS 023N low-power polling compatible with tested VX-6R, VX-7R, and VX-8R radios while retaining normal voice RX and PTT voice TX. A repeating three-tone alarm warns of a lost link, and two high tones announce recovery.

The public firmware contains no personal callsign. Rescue teams can configure each radio individually or deploy a CSV roster with `team_config.py`. The tool backs up the previous value, writes only the dedicated eight-byte SAR-TEAM EEPROM block, reads it back, and verifies CRC-8.
